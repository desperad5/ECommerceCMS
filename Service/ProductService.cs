...
namespace SorubankCMS.Service
{
    public class ProductService : IProductService
    {
        ...
        private readonly ISorubankUserRepository _userRepository;
        private readonly IProductCommentRepository _productCommentRepository;
        private readonly IMapper _mapper;
        private static readonly ILog logger = Logger.GetLogger(typeof(ProductService));

        public ProductService(IProductRepository productRepository, ITopicCardRepository topicCardRepository, IQuestionCardRepository questionCardRepository, IExamCardRepository examCardRepository, ISorubankUserRepository userRepository,IProductCommentRepository productCommentRepository,IMapper mapper)
        {
        ...
        public ServiceResult<List<ProductCommentModel>> GetProductComments(int productId)
        {
            ...
            try
            {
                ...
                result.data = product.ProductComments.OrderByDescending(t=>t.CreatedDate).Select(i => {
                    var user = _userRepository.GetSingle(t => t.Id == i.SoruBankUserId);
                    return new ProductCommentModel()
                    {
                        ...
                        Id = i.Id,
                        User = new UserViewModel() { Id = i.SoruBankUserId,UserName=user.UserName, Name=user.FirstName, Surname=user.LastName }
                    };
                }).ToList();
                result.resultType = ServiceResultType.Success;
                return result;
